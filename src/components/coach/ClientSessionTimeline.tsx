...
export function ClientSessionTimeline({ items, clientCrmId }: ClientSessionTimelineProps) {
  if (items.length === 0) {
    return (
  ...
  return (
    <div className="space-y-3">
      {items.map((item) => {
        ...
        <Card key={`${item.type}-${item.id}`} className={cn('border border-border', isClickable && 'hover:border-[#20808D]/50 hover:bg-[#E8F4F5]/30 transition-colors cursor-pointer')}>
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex flex-col items-end gap-1 shrink-0">
                <div className="flex items-center gap-1">
                  ...
                  {isClickable && <ChevronRight className="h-3.5 w-3.5 text-[#20808D]" />}
                </div>
                      ...
                      <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                        <div
                          className={cn(
                            ...
                            pct === 100 ? 'bg-green-500' : 'bg-[#20808D]',
                          )}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
