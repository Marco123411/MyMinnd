...
export function CoachGuidePanel({ profile }: CoachGuidePanelProps) {
  return (
    <div className="rounded-xl overflow-hidden border border-[#141325]/20">
      {/* En-tête coach — fond sombre, accès restreint */}
      <div className="flex items-center gap-2 bg-[#141325] px-4 py-3">
        <Shield className="h-4 w-4 text-[#FF9F40]" />
      ...
      <div className="bg-white divide-y divide-gray-100">
        ...
        {profile.coach_priority && (
          <div className="flex items-start gap-3 px-4 py-3">
            <Target className="h-4 w-4 mt-0.5 shrink-0 text-[#7069F4]" />
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-0.5">
                Priorité #1
              </p>
              <p className="text-sm text-[#141325] font-medium">{profile.coach_priority}</p>
            </div>
          </div>
        )}
        ...
        {profile.coach_exercise && (
          <div className="flex items-start gap-3 px-4 py-3">
            <Dumbbell className="h-4 w-4 mt-0.5 shrink-0 text-[#7069F4]" />
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-0.5">
                Exercice recommandé
              </p>
              <p className="text-sm text-[#141325]">{profile.coach_exercise}</p>
            </div>
          </div>
        )}
        ...
        {profile.coach_trap && (
          <div className="flex items-start gap-3 px-4 py-3">
            <AlertTriangle className="h-4 w-4 mt-0.5 shrink-0 text-[#3C3CD6]" />
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-0.5">
                Piège à éviter
              </p>
              <p className="text-sm font-medium text-[#3C3CD6]">{profile.coach_trap}</p>
            </div>
          </div>
        )}
        ...
        {(profile.team_role || profile.team_contribution) && (
          <div className="flex items-start gap-3 px-4 py-3">
            <Users className="h-4 w-4 mt-0.5 shrink-0 text-[#7069F4]" />
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-0.5">
                Rôle en équipe
              </p>
              {profile.team_role && (
                <p className="text-sm font-medium text-[#141325]">{profile.team_role}</p>
              )}
